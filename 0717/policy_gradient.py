...
class Agent():
    ...
    def sampleAction(self, s):
        ...
        # if len(shape) == 1:
        #     assert shape == (self.s_dim,), "{} != {}".format(shape, self.s_dim)
        #     s = np.expand_dims(s, axis=0)

        # elif len(shape) == 2:
        #     assert shape[1] == (self.s_dim), "{} != {}".format(shape, self.s_dim)

        # else:
        #     raise TypeError("Wrong state shape is given: {}".format(s.shape))

        action_prob = np.squeeze(self.model.predict(s))
        # assert len(action_prob) == self.a_dim, "{} != {}".format(len(action_prob), self.a_dim)
        return np.random.choice(np.arange(self.a_dim), p=action_prob)

    def storeSample(self, s, a, r):
        #  TODO: 儲存一個 episode 中所有的(s,a,r)pair, 存到_batch
        a_one_hot = np_utils.to_categorical(a, num_classes=self.a_dim)
        ...
        self.a_batch[self.batch_counter] = a_one_hot
...
if __name__ == '__main__':
    # 參數設定
    EPISODE = 100  # 要玩幾個回合
    N = 30  # sample 多少個回合才 update model 一次
